    ...
    def _openai_json_response(
        ...
        *,
        ...
        try:
            ...
            log_event(logger, "Calling OpenAI JSON model.", model=selected_model, prompt_chars=len(prompt))
            ...
            if parsed is None:
                log_event(logger, "OpenAI JSON model returned invalid JSON.", model=selected_model)
                return self._fallback_or_raise(fallback, "OpenAI model returned invalid JSON.")
            log_event(logger, "OpenAI JSON model completed.", model=selected_model, keys=len(parsed))
            return ModelResult(data=parsed, used_model=True)
        except Exception as exc:
            log_event(logger, "OpenAI JSON model failed.", error=str(exc))
    ...
    def _openai_image_json_response(
        ...
        *,
        ...
        try:
            ...
            parsed = self._extract_json(getattr(response, "output_text", "") or "")
            if parsed is None:
                log_event(logger, "OpenAI vision model returned invalid JSON.", model=self.vision_model)
                return self._fallback_or_raise(fallback, "OpenAI vision model returned invalid JSON.")
            log_event(logger, "OpenAI vision model completed.", model=self.vision_model, keys=len(parsed))
            return ModelResult(data=parsed, used_model=True)
        except Exception as exc:
            log_event(logger, "OpenAI vision model failed.", error=str(exc))
